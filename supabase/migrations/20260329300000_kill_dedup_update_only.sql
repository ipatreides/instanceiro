-- Add p_update_only parameter: when true, only update existing kills (don't create new ones)
-- Used by mvp-killer for stale tombs — we still want the killer name, but shouldn't create
-- phantom kills from old tombstones
...
BEGIN
  ...
  -- update_only mode: no existing kill found, don't create a new one
