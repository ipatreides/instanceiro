-- Fix: get_group_active_kills should return the kill with the most recent
-- killed_at, not created_at. A kill can be created early (sentinel) and
-- updated later with the real killed_at from the tomb. Using created_at
-- picks the wrong kill when an older sentinel gets updated after a newer
-- manual kill was registered.
--
-- Use killed_at DESC with sentinels (epoch 0) sorted last.
