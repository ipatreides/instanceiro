-- Fix kill dedup: widen window, use advisory lock, prevent race conditions
--
-- Problems fixed:
-- 1. 30-second dedup window too tight (mvp-kill uses exact timestamp, mvp-killer uses hour:minute)
-- 2. FOR UPDATE SKIP LOCKED lets concurrent requests bypass dedup
-- 3. No protection against stale tomb data creating kills
...
BEGIN
  -- Serialize all kill registrations for the same group+mvp to prevent race conditions
  PERFORM pg_advisory_xact_lock(hashtext(p_group_id::text || p_mvp_ids[1]::text));

  -- Widen dedup window to 5 minutes (handles mvp-kill vs mvp-killer timestamp differences)
  v_dedup_cutoff := p_killed_at - INTERVAL '5 minutes';

  -- Atomic dedup: check if kill exists within window
