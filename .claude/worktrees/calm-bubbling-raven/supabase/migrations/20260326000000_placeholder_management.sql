...
-- RPC: get_schedule_placeholders
-- Returns placeholders with enriched character + profile data
-- Uses SECURITY DEFINER to bypass characters RLS (friends' chars not readable via RLS)
CREATE OR REPLACE FUNCTION get_schedule_placeholders(p_schedule_id UUID)
RETURNS JSON AS $$
DECLARE
  v_result JSON;
BEGIN
  SELECT json_agg(row_to_json(t) ORDER BY t.created_at) INTO v_result
  FROM (
    SELECT
      sp.id,
      sp.schedule_id,
      ...
      sp.added_by,
      sp.claimed_by,
      sp.claimed_character_id,
      sp.created_at,
      CASE WHEN sp.claimed_character_id IS NOT NULL THEN
        json_build_object('name', c.name, 'class', c.class, 'level', c.level)
      ELSE NULL END AS characters,
      CASE WHEN sp.claimed_by IS NOT NULL THEN
        json_build_object('username', p.username, 'avatar_url', p.avatar_url)
      ELSE NULL END AS profiles
    FROM schedule_placeholders sp
    LEFT JOIN characters c ON c.id = sp.claimed_character_id
    LEFT JOIN profiles p ON p.id = sp.claimed_by
