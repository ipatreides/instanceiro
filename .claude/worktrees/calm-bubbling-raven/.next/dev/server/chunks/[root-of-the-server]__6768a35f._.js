...
async function POST(request) {
    ...
    await supabase.from('telemetry_pairing_requests').delete().lt('expires_at', new Date().toISOString());
    const pairingCode = generatePairingCode();
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000).toISOString();
    const { error } = await supabase.from('telemetry_pairing_requests').insert({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6768a35f._.js.map