...
"[project]/instance-tracker/.claude/worktrees/calm-bubbling-raven/src/app/api/telemetry/mvp-killer/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function POST(request) {
    ...
    if (!killer_name) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$instance$2d$tracker$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            error: 'Missing killer_name'
        }, {
            status: 400
        });
    }
    // Find kill by tomb coordinates + map in this group
    let query = supabase.from('mvp_kills').select('id').eq('group_id', ctx.groupId);
    if (tomb_x != null && tomb_y != null) {
        query = query.eq('tomb_x', tomb_x).eq('tomb_y', tomb_y);
    }
    const { data: kill } = await query.order('killed_at', {
        ascending: false
    }).limit(1).maybeSingle();
    if (!kill) {
        return __TURBOPACK__imported__module__$5b$project$5d2f$instance$2d$tracker$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
            action: 'ignored',
            reason: 'no matching kill'
        });
    }
    // Try to resolve killer_name to a character_id in the group
    const { data: members } = await supabase.from('mvp_group_members').select('character_id, characters!inner(name)').eq('group_id', ctx.groupId);
    const match = members?.find((m)=>m.characters?.name === killer_name);
    const updates = {
    ...
    await supabase.from('mvp_kills').update(updates).eq('id', kill.id);
    return __TURBOPACK__imported__module__$5b$project$5d2f$instance$2d$tracker$2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        action: 'updated',
        kill_id: kill.id,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__3e757ff9._.js.map